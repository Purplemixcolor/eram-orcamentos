...
export default function DashboardPage() {
  ...
  const topCategories = categories.slice(0, 6).map((category) => ({
    category,
      ...
      <div className="space-y-6 p-4 lg:p-6">
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          {[
            ...
            return (
              <article key={item.label} className="rounded-md border border-[#d8e1ec] bg-white p-4 shadow-sm">
                ...
                <div className="mt-4 text-2xl font-bold text-[#0b2e59]">{item.value}</div>
            ...
            <div className="mt-4 space-y-3">
              {byYear.map((item) => (
                <div key={item.year} className="grid grid-cols-[56px_1fr_32px] items-center gap-3 text-sm">
                  ...
                  <span className="h-2 rounded bg-[#e6edf5]"><span className="block h-2 rounded bg-[#1e5a8a]" style={{ width: `${Math.max(item.count * 12, 8)}%` }} /></span>
          ...
          <div className="rounded-md border border-[#d8e1ec] bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {serviceRecords.slice(0, 5).map((service) => (
                <Link key={service.id} href={`/services/${service.id}`} className="focus-ring block rounded border border-[#edf2f7] p-3 hover:bg-[#f8fafc]">
                  <div className="font-semibold text-[#123c72]">{service.title}</div>
          ...
          <div className="rounded-md border border-[#d8e1ec] bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {topCategories.map((item) => (
                <div key={item.category} className="flex items-center justify-between rounded border border-[#edf2f7] px-3 py-2 text-sm">
